...
│   └── decision_log.py               # NEW: historical decisions
├── risk/
...
CREATE TABLE decisions (
    ...
    execution_status TEXT,  -- 'paper' | 'live' | 'blocked_by_risk_gate'
    block_reason TEXT
);
...
### Phase 5: Orchestration (target: 1 week)
...
**Exit criteria**: Schedule daily run at 8:00 EST, decisions appear in Gmail inbox, cost tracked in SQLite.

### Phase 6: Backtest (target: 2-4 weeks)
...
**Exit criteria**: 12 months of backtest complete. SR > 1.5 AND MDD < 25% to proceed to live.

### Phase 7: Paper trading (target: 30 days)

- [ ] Connect to moomoo paper trading account
- [ ] Run daily for 30 trading days
- [ ] Daily report to Gmail with positions, P&L, and agent rationale
- [ ] Review all vetoed proposals (weekly)
- [ ] Tune prompts and weights based on paper performance

**Exit criteria**: 30-day paper Sharpe > 1.0 AND max drawdown < 15%. User approval required before Phase 8.

### Phase 8: Small live (target: ongoing)
